...
#or tell the user that he got the number correct.  The user should continue 
#to make guesses until he guesses correctly. Also, once the user guesses correctly, 
...
def guess()
  ...
  @answer = ((@max-@min)/2 + @min).round
  puts @answer
  if @answer < @number
    puts "The number is higher than #{@answer}. Guess again"
    @min = @answer + 1 
    @guess_count += 1
  elsif @answer > @number
    puts "The number is lower than #{@answer}. Guess again"
    @max = @answer - 1
    @guess_count += 1
  elsif @answer == @number
